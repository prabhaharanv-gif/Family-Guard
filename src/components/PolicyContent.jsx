  ...
  return (
    <div>
      <div style={{
        ...
        <div style={{ fontSize: 14, fontWeight: 800, color: '#2A0A18', marginBottom: 6 }}>
          {p.consentTitle}
        </div>
      ...
      {p.sections.map(s => (
        <div key={s.key} style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 800, color: '#2A0A18' }}>{s.title}</span>
          </div>
          {s.items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'flex-start' }}>
              ...
              <div style={{ fontSize: 13, color: '#4A1226', lineHeight: 1.5 }}>{item}</div>
            </div>
          ))}
        </div>
      ))}
      <div style={{
        background: '#fff', borderRadius: 16, padding: '14px 16px',
        textAlign: 'center', border: '1px solid #ECE0E5',
      }}>
        <div style={{ fontSize: 13, color: '#4A1226', lineHeight: 1.55 }}>
          {p.consentNote}
        </div>
