...
/**
 * Profile → Shake for SOS.
 *
 * One switch. Off unless the member turns it on: a gesture that can alert every
 * family must be a choice. Turning it on explains, once, exactly what happens,
 * so nobody finds out by setting it off in a bag. Android only.
 */
export default function ShakeSosCard({ Toggle }) {
  const t = useT()
  ...
  const toggle = async () => {
    ...
    setEnabled(next)
    try {
      await LocationService.setShakeSos({ enabled: next })
  ...
  return (
    <div className="settings-card" style={{ marginBottom: 10, padding: '14px 16px' }}>
      ...
      {explain && (
        <Dialog type="info" title={t('shakeSos.title')} message={t('shakeSos.howItWorks')}
